...
class TrackLabeler(Actor):   
    ...
    def process_keys(self,symbol,modifiers):
        ...
        if symbol == key.P:            
            ...
            #self.save_bundles_temp()
            
        if symbol==key.E:
            print 'E'
            #first to save current choosing track for backup
	    ...
	    self.save_bundles_temp()
            ...
            root = Tkinter.Tk()
            root.withdraw()
            pickle.dump(self.tracks_ids_to_be_saved, tkFileDialog.asksaveasfile(parent=root,initialdir='Tracts'), protocol=pickle.HIGHEST_PROTOCOL)
...
"""
    ...
    def save_bundles_temp(self):
        from dipy.io.pickles import save_pickle        
        print('Save selected tracks_ids as pickle file in a temp file.')
        bundles_file = 'bundles_temp.pkl'
	select_bundles_file = 'select_bundles_temp.pkl'
        bundles_ids_to_be_saved_temp = []#self.tracks_ids
        if len(self.selected)>0:
            bundles_ids_to_be_saved_temp = self.selected            
        save_pickle(select_bundles_file,bundles_ids_to_be_saved_temp)
        save_pickle(bundles_file,self.qb)
