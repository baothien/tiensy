...
if __name__ == '__main__':
    ...
    for a in ['L','R']:
        features_volumn =[]
        features_density =[]
        features_vol_name = dir_name + '/segmentation/patient_fiber_volumn_corticospinal_' + a + '_3M.txt'          
        features_den_name = dir_name + '/segmentation/patient_fiber_density_corticospinal_' + a + '_3M.txt'   
        
        #features_vol_name = dir_name + '/segmentation/control_fiber_volumn_corticospinal_' + a + '_3M.txt'    
        #features_den_name = dir_name + '/segmentation/control_fiber_density_corticospinal_' + a + '_3M.txt'   
        ...
        #for p in controls:        
            ...
            features_volumn.append(volumn)
            
            #compute fiber density = number_fibers/volumn
            fiber_density =  number_fibers*1./volumn  
            features_density.append(fiber_density)
        
            #print cordinate_file_name, '\t', number_fibers, '\t', volumn, '\t', fiber_density
            
        save_pickle(features_vol_name,features_volumn)
        save_pickle(features_den_name,features_density)
                
        #for test and load later        
        print features_vol_name
        features=load_pickle(features_vol_name)
        print features
        
        print features_den_name
        features=load_pickle(features_den_name)
        print features