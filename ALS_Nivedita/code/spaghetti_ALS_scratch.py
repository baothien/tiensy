...
if __name__ == '__main__':
    ...
    root = Tkinter.Tk()
    root.wm_title('Subject Selection - remember to chose the correct subject')
    ...
    fpkl='bundles_temp.pkl'
    qb=load_pickle(fpkl)
    #visualisation part        
    tl = TrackLabeler(qb,qb.downsampled_tracks(),vol_shape=data.shape,tracks_alpha=1,virtuals_line_width=5.0)
#add new 
    selected_bundles_fname = 'select_bundles_temp.pkl' 
       
    bundles_selected = load_pickle(selected_bundles_fname)
    tl.select_track(ids for ids in bundles_selected)    
#finish     
