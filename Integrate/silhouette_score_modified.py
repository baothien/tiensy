# -*- coding: utf-8 -*-
"""
Created on Thu May  9 14:37:02 2013

@author: bao
Thank Alexandre Araham @ https://gist.github.com/AlexandreAbraham/ for sharing this code
Source:  https://gist.github.com/AlexandreAbraham/5544803

Methods :
- *_slow : these functions implement the "compute distance on the fly method".
- *_block : the smarter method. Basically, distance matrices are computed per cluster.
...
- big data (X = np.random.random((20000, 1000)), y = np.repeat(np.arange(100), 200)):
...
- block method is a little slower but uses far less memory. This, obviously, depends on your cluster sizes.
"""
...
def silhouette_score_block(X, labels, metric='euclidean', sample_size=None,
    ...
    if sample_size is not None:
        ...
        if metric == "precomputed":
            ...
            X, labels = X[indices], labels[indices]
    return np.mean(silhouette_samples_block(X, labels, metric=metric, **kwds))
 
 
def silhouette_samples_block(X, labels, metric='euclidean', **kwds):
    """Compute the Silhouette Coefficient for each sample.
    ...
    """
    A = _intra_cluster_distances_block(X, labels, metric, **kwds)
    B = _nearest_cluster_distance_block(X, labels, metric, **kwds)
    sil_samples = (B - A) / np.maximum(A, B)
    # nan values are for clusters of size 1, and should be 0
    return np.nan_to_num(sil_samples)
 
 
def _intra_cluster_distances_block(X, labels, metric, **kwds):
    """Calculate the mean intra-cluster distance for sample i.
    ...
    for label in np.unique(labels):
        ...
        intra_dist[indices] = distances.sum(axis=1) / (distances.shape[0] - 1)
    return intra_dist
 
 
def _nearest_cluster_distance_block(X, labels, metric, **kwds):
    """Calculate the mean nearest-cluster distance for sample i.
...
if __name__ == '__main__':
    ...
    t = time.time() - t0
    print 'Scikit silhouette (%ds): %f' % (t, s)
    t0 = time.time()
    s = silhouette_score_slow(X, y)
    t = time.time() - t0
    print 'Slow silhouette (%ds): %f' % (t, s)
    t0 = time.time()
    s = silhouette_score_block(X, y)
    t = time.time() - t0
    print 'Block silhouette (%ds): %f' % (t, s)
'''
#---------------------------------------------------------------------------