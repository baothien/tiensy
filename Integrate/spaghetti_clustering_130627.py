...
import pickle
from streamshow_130627 import StreamlineLabeler, compute_buffers, compute_buffers_representatives, mbkm_wrapper
...
if __name__ == '__main__':
    ...
    subject = '101'
    num_M_seeds = 1
    directory_name='ALS_Data/'
    qb_threshold = 30 # in mm
    qb_n_points = 12
    downsampling = False

    #load T1 volume registered in MNI space
    t1_filename = directory_name + subject+'/MP_Rage_1x1x1_ND_3/T1_flirt_out.nii.gz'
    ...
    tracks_basenane = directory_name+ subject +'/DIFF2DEPI_EKJ_64dirs_14/DTI/tracks_dti_'+str(num_M_seeds)+'M_linear' 
...
#            T = [np.dot(t, rotation_matrix(axis, theta)) for t in T]
    ...
    n_clusters = 150
    clusters_filename = directory_name+ subject+'/DIFF2DEPI_EKJ_64dirs_14/DTI/mbkm_dti_'+str(num_M_seeds)+'M_linear_'+str(n_clusters)+'_clusters.pickle'
    try:
