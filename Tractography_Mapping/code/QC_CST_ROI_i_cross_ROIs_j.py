# -*- coding: utf-8 -*-
"""
Created on Mon May 12 16:13:02 2014
...
def intersec_two_ROIs(tracks, ROIs, Rs, vis = False):
...
Rs = [4.,4.]
visualize = False#True#False#True# False
...
for s_id in np.arange(len(source_ids)):
    ...
    print 'Source: ', source, '\t', len(tracks) 
    ...
    for t_id in np.arange(len(target_ids)):
        ...
        t1, t2, common = intersec_two_ROIs(tracks, ROIs, Rs, vis = visualize)
        
        
        #print '\t', target, '\t', len(t1), '\t', len(t2), '\t', len(common)
        print len(t1), '\t', len(t2), '\t', len(common)
