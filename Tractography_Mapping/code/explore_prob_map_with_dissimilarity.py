    ...
    for s_id in np.arange(len(source_ids)):
        ...
        for t_id in np.arange(len(target_ids)):        
            if (target_ids[t_id] != source_ids[s_id]):                
                ...
                #--------------------------------------------------
                #only for probability mapping
                #load map and chose the highest prob
                
                map_tmp = load_pickle(map_file)
                
                from common_functions import init_prb_state_sparse
            
                prb_map12_init, cs_idxs = init_prb_state_sparse(s_cst_sff_in_ext,t_cst_ext,nn)                 
                 
                map_idxs = [map_tmp[i].argsort()[-1] for i in np.arange(len(s_cst_sff_in_ext))]                
                
                map_all = [cs_idxs[i][map_idxs[i]] for i in np.arange(len(s_cst_sff_in_ext))]
                #---------------------------------------------------
                
                '''
                #---------------------------------------------------------
                #if map is not probability   
