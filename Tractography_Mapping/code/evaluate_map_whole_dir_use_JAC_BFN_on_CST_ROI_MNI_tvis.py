...
with each fiber index i in source, mapp[i] is the index of fiber in target
      """
from common_functions import load_tract, Jac_BFN, Jac_BFN_1, visualize_tract
...
for a_id in np.arange(len(anneal)):
    ...
    print "\t\t Target \t not_map_Jac \t not_map_BFN \t map_Jac \t map_BFN \t 1NN_same_ANN \t 1NN_diff_ANN" 
    for s_id in np.arange(len(source_ids)):
        ...
        for t_id in np.arange(len(target_ids)):        
            if (target_ids[t_id] != source_ids[s_id]):                
                ...
                map_file_1nn = '/home/bao/tiensy/Tractography_Mapping/data/trackvis_tractography/results/result_cst_sff_in_ext_2_cst_ext/50_SFF_MNI/map_1nn_' + source + '_' + target + '_cst_L_ann_100_MNI.txt'
                '''
                ...
                print "\t\t", target_ids[t_id],"\t",  cst_len, "\t",  same, "\t", diff
