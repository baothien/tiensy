...
def visualize(ren, tract1, tract2, mapping):
    ...
    #fvtk.add(r,c)
    colors = [fvtk.red,fvtk.green, fvtk.blue, fvtk.white,fvtk.yellow, fvtk.cyan,fvtk.dark_blue,fvtk.dark_green,fvtk.dark_red,fvtk.gray,fvtk.golden,fvtk.hot_pink]
    for i in np.arange(len(tract1)):
        fvtk.add(ren, fvtk.line(tract1[i], colors[i], opacity=1.0))
        fvtk.add(ren, fvtk.line(tract2[mapping[i]], colors[i], opacity=1.0))
...
if __name__ == '__main__':
    ...
    dis_1, pro_1 = compute_disimilarity(tracks_1, bundles_distances_mam, prototype_policies[2], num_prototypes=size_1)#,tracks_1.size)
    dis_2, pro_2 = compute_disimilarity(tracks_2, bundles_distances_mam, prototype_policies[2], num_prototypes=size_2)#,tracks_2.size)
    ...
    def loss_function2(mapping12):
        ...
        for sid in np.arange(len(pro_1)):               
            ...
            #idx2 = kdt_2.query_radius(mapping12[sid], radius)[0]
    ...
    ren = fvtk.ren() 
    ren = visualize(ren,tractography1,tractography2,mapping12_best)
    fvtk.show(ren)
